...
with open(r"C:\Users\carlosromero\Desktop\PMA_REVIT_CARLOS.csv", "r") as csvfile:
    ...
    for row in reader:
        if row[0]:
            PMArooms_from_PMA.append(PMAroom(eliminarAcentos(row[0]),eliminarAcentos(row[2]),eliminarAcentos(row[4]),eliminarAcentos(row[6]),eliminarAcentos(row[8]),eliminarAcentos(row[10]),float(row[13].decode('utf-8').replace(",","."))))
...
def PMAroom_from_Revit(room):
    def pStr(el, name):
        if el.GetParameters(name)[0].AsString():
            return eliminarAcentos(el.GetParameters(name)[0].AsString())
        else:
            return ""
