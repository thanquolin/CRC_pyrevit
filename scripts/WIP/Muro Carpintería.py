# coding: utf8
"""Crea un muro con cada tipo de puerta y ventana en el proyecto en él, desde el exterior, con el nombre del tipo, y alberga el alzado de éste en la Sheet 999-Memoria Carpintería."""
...
walltypeid = FilteredElementCollector(doc).OfCategory(Autodesk.Revit.DB.BuiltInCategory.OST_Walls).WhereElementIsElementType().ToElements()[0].Id
levelid = FilteredElementCollector(doc).OfClass(Level).ToElements()[0].Id
curve = Line.CreateBound(point, XYZ(point.X + 5,0,0))
t = Transaction(doc,"Wall")
t.Start()
Wall.Create(doc,curve,walltypeid,levelid,5,0.0,False,False)
t.Commit()
...
#Transacciones para aislar la carpintería en cada sección OPCIÓN: CREAR UNA VIEW TEMPLATE Y ASIGNARLA A LAS SECCIONES
for view, plan in zip(new_views, new_plans):
